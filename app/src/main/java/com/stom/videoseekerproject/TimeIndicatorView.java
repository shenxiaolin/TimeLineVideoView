...
public class TimeIndicatorView extends View implements View.OnTouchListener {
    ...
    public static interface OnSeekListener {
        void onTimeIndicatonSeek(float time);
    }
    ...
    public float getCurrentTime() {
        return position / (right - left) * getDuration();
    }
    ...
    private void invalidateSize() {
        left = indicatorWidth / 2 + (getWidth() - indicatorWidth) * startTime / MAX_DURATION;
        right = indicatorWidth / 2 + (getWidth() - indicatorWidth) * endTime / MAX_DURATION;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRect(left, 0f, right, getHeight(), backgroundPaint);
        drawScale(canvas);
        drawIndicator(canvas);
    }

    private void drawScale(Canvas canvas) {
        float secondSize = (right - left) / (endTime - startTime);

        Path path = new Path();
        path.moveTo(left, getHeight() - SCALE_STROKE_WIDTH / 2f);
        path.lineTo(right, getHeight() - SCALE_STROKE_WIDTH / 2f);
        for (int i = 1; i * secondSize < right - left; i++) {
            path.moveTo(left + i * secondSize, getHeight());
            path.lineTo(left + i * secondSize, getHeight() - (i % 5 == 0 ? scaleHeight * 2 : scaleHeight));
        }
        path.close();
    ...
    private void drawIndicator(Canvas canvas) {
        Path path = new Path();
        path.moveTo(left + position, getHeight());
        path.lineTo(left - indicatorWidth / 2f + position, 0);
        path.lineTo(left + indicatorWidth / 2f + position, 0);
        path.lineTo(left + position, getHeight());
        path.close();
